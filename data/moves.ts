...
export const Moves: import('../sim/dex-moves').MoveDataTable = {
	...
	fakeout: {
		...
		onTry(source, target) {
			...
			if (target.wasFakedOut) {
				this.hint("Fake Out can only affect a Pokemon once per game.");
				return false;
			}
			target.wasFakedOut = true;
		},
		onDisableMove(pokemon) {
	...
	minimize: {
		...
		onTry(source, target, move) {
			if (source.boosts.evasion > 0) {
				this.hint('Minimize will fail if you already have Evasion boosts.');
				return;
			};
		},
	},
	...
	pinmissile: {
		...
		contestType: "Cool",
		onDamage(damage, target, source, effect) {
			const weakElectric = target.runEffectiveness(this.dex.getActiveMove('thunderbolt')) > 1;
			if (source.baseSpecies.name === 'Jolteon' && weakElectric) {
				return damage * 2;
			};
		},
	},
	...
	powertrip: {
		...
		basePower: 20,
		basePowerCallback(pokemon, target, move) {
			let bp = move.basePower + 20 * pokemon.positiveBoosts();
			if (bp > 100) bp = 100;
			this.debug(`BP: ${bp}`);
			return bp;
		},
	...
	stealthrock: {
		...
		condition: {
			...
			onSwitchIn(pokemon) {
				...
				// fuck this
				if (typeMod === 0.25) typeMod = 1;
				else if (typeMod === 0.5) typeMod = 2.5;
				else if (typeMod === 1) typeMod = 5;
				else if (typeMod === 2) typeMod = 7.5;
				else if (typeMod === 4) typeMod = 10;
				else throw new Error(`stealthrock damage invalid damage mult ${typeMod}`);
				this.damage(typeMod / 100);
			},
		},
		target: "foeSide",
		type: "Rock",
	...
	stuffcheeks: {
		...
		onHit(pokemon) {
			if (!this.boost({ def: 4 })) return null;
	...
	syrupbomb: {
		...
		condition: {
			...
			onResidualOrder: 14,
			onResidual(pokemon) {
				this.boost({ spe: -2 }, pokemon, this.effectState.source);
			},
			onEnd(pokemon) {
	...
	thief: {
		...
		onAfterHit(target, source, move) {
			if (source.item || source.volatiles['gem']) {
				const item = target.takeItem();
				if (item) {
					this.add('-enditem', target, item.name, '[from] move: Thief', `[of] ${source}`);
				}
			}
			const yourItem = target.takeItem(source);
			if (!yourItem) {
	...
	watershuriken: {
		...
		basePowerCallback(pokemon, target, move) {
			...
			if (pokemon.species.name === 'Greninja-Mega') return 75;
			return move.basePower;
		},
		category: "Special",
		name: "Water Shuriken",
		...
		multihit: [2, 5],
		onModifyMove(move, pokemon, target) {
			if (pokemon.species.name === 'Greninja-Mega') move.multihit = undefined;
		},
		target: "normal",
		type: "Water",
		contestType: "Cool",
